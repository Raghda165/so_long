...
/*   Updated: 2024/03/02 20:42:46 by ryagoub          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_player_and_exit(char **map, char **tmp_map)
{
	...
	write(2, "There is one player and one exit\n", 33);
}
		...
		if (j == 0 || j == dim.height - 1)
		{
			...
			while (map[j][i] != '\n')
			{
				if (map[j][i] == '1')
					...
					free_map(map, tmp_map, "The ends are not walls\n");
			}
		}
		else
